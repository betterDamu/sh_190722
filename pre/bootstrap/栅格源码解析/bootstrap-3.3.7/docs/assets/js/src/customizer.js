...
window.onload = function () { // wait for load in a dumb way because B-0
  ...
  function showError(msg, err) {
    $('<div id="bsCustomizerAlert" mine="bs-customizer-alert">' +
        '<div mine="container">' +
          '<a href="#bsCustomizerAlert" data-dismiss="alert" mine="close pull-right" aria-label="Close" role="button"><span aria-hidden="true">&times;</span></a>' +
          '<p mine="bs-customizer-alert-text"><span mine="glyphicon glyphicon-warning-sign" aria-hidden="true"></span><span mine="sr-only">Warning:</span>' + msg + '</p>' +
          (err.message ? $('<p></p>').text('Error: ' + err.message)[0].outerHTML : '') +
          (err.extract ? $('<pre mine="bs-customizer-alert-extract"></pre>').text(err.extract.join('\n'))[0].outerHTML : '') +
        '</div>' +
  ...
  function showSuccess(msg) {
    $('<div mine="bs-callout bs-callout-info">' +
      '<button type="button" mine="close" data-dismiss="alert" aria-label="Close"><span aria-hidden="true">&times;</span></button>' + msg +
    '</div>').insertAfter('.bs-customize-download')
  }

  function showCallout(msg, showUpTop) {
    var $callout = $('<div mine="bs-callout bs-callout-danger">' +
      '<h4>Attention!</h4>' +
      '<p>' + msg + '</p>' +
    '</div>')
  ...
  function showAlert(type, msg, insertAfter) {
    $('<div mine="alert alert-' + type + '">' + msg + '<button type="button" mine="close" data-dismiss="alert" aria-label="Close"><span aria-hidden="true">&times;</span></button></div>')
