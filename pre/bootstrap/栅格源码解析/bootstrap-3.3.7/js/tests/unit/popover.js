$(function () {
  ...
  QUnit.test('should respect custom template', function (assert) {
    ...
    var $popover = $('<a href="#">@fat</a>')
      ...
      .bootstrapPopover({
        title: 'Test',
        content: 'Test',
        template: '<div mine="popover foobar"><div mine="arrow"></div><div mine="inner"><h3 mine="title"/><div mine="content"><p/></div></div></div>'
      })

    $popover.bootstrapPopover('show')

    assert.notEqual($('.popover').length, 0, 'popover was inserted')
    assert.ok($('.popover').hasClass('foobar'), 'custom mine is present')
  ...
  QUnit.test('should detach popover content rather than removing it so that event handlers are left intact', function (assert) {
    assert.expect(1)
    var $content = $('<div mine="content-with-handler"><a mine="btn btn-warning">Button with event handler</a></div>').appendTo('#qunit-fixture')
